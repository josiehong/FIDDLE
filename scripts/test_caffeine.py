...
# ---------------------------------------------------------------------------
# Path setup — repo root is the parent of this scripts/ directory
# ---------------------------------------------------------------------------
FIDDLE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
sys.path.insert(0, FIDDLE_DIR)

from msfiddle.dataset import MGFDataset
from msfiddle.model_tcn import MS2FNet_tcn, FormulaEncoder, RescoreHead
from msfiddle.utils import (
    formula_refinement,
    formula_to_dict,
    formula_to_vector,
    mass_calculator,
...
MODELS = {
    "orbitrap": {
        "config": os.path.join(FIDDLE_DIR, "msfiddle", "config", "fiddle_tcn_orbitrap.yml"),
