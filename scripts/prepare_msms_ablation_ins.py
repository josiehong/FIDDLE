...
from msfiddle.utils import sdf2mgf, filter_spec, mgf_key_order, spec2arr, spec2pair
from msfiddle.utils import mgf_key_order
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pkl_dir", type=str, default="./data/ablation_ins/", help="Path to pkl data"
    )
    parser.add_argument(
        "--train_ratio",
        type=float,
    ...
    parser.add_argument(
        "--config_qtof_path",
        type=str,
        default="./msfiddle/config/fiddle_tcn_qtof.yml",
        help="Path to QTOF configuration",
    )
    parser.add_argument(
        "--config_orbitrap_path",
        type=str,
        default="./msfiddle/config/fiddle_tcn_orbitrap.yml",
    ...
    print(
        f"Found {len(overlapped_compounds)} overlapped compounds between Q-TOF and Orbitrap"
    )

    # Remove the spectra from non-overlapped compounds
    mona_qtof_spectra = [
        spectrum
        for spectrum in mona_qtof_spectra
        if spectrum["params"]["smiles"] in overlapped_compounds
    ]
    nist20_spectra = [
        spectrum
        for spectrum in nist20_spectra
        if spectrum["params"]["smiles"] in overlapped_compounds
    ]
