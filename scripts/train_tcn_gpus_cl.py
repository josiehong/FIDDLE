...
# Make the repo root importable so `msfiddle` resolves without an editable install.
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from msfiddle.dataset import MS2FDataset_CL, MS2FDataset
from msfiddle.model_tcn import MS2FNet_tcn
from msfiddle.utils import ATOMS_WEIGHT, ATOMS_INDEX, ATOMS_INDEX_re, vector_to_formula
...
if __name__ == "__main__":
    ...
    # Early stopping keeps runs short (tens of epochs), so StepLR(step_size=100) never
    # fired and the LR stayed pinned at its initial value. Use ReduceLROnPlateau so the
    # LR halves when validation mass error plateaus, before early stop triggers.
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.5, patience=config["train"]["patience"]
    )
    ...
    for epoch in range(epoch_start + 1, config["train"]["epochs"] + 1):
        ...
        # Early stopping & LR schedule are driven by validation mass error (smooth);
        # the with-H formula accuracy is too noisy/low to drive stopping reliably.
        if valid_mae < best_valid_mae:
            best_valid_mae = valid_mae
            early_stop_patience = 0
            print("Early stop patience reset (valid_mae improved)")
        else:
        ...
        # Checkpoint selection is unchanged: keep the model with the best formula
        # accuracy (with H). Decoupled from early stopping above.
        if best_formula_acc < formula_acc:
            best_formula_acc = formula_acc
            best_formula_wo_acc = formula_wo_acc

            if args.checkpoint_path != "":
                print("Saving checkpoint (best formula acc)...")
                checkpoint = {
                    "epoch": epoch,
        ...
        # Step the LR scheduler after the warmup phase (halve LR when valid_mae plateaus)
