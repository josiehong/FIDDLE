...
- `test_caffeine.py`, `test_caffeine_qtof.py`, `test_caffeine_orbitrap.py`: inference debugging scripts for caffeine (C8H10N4O2) covering GNPS, NIST20/23, MoNA, and Agilent spectra.
...
- `train_rescore.py`: removed `pos_weight=4.0` from `BCEWithLogitsLoss` now that the augmented dataset is 1:1 balanced.
...
- `prepare_rescore.py`, `run_fiddle.py`: `refine_atom_type` is now extended with atoms present in the predicted formula before calling `formula_refinement`, ensuring the refinement search space at training time matches inference.
