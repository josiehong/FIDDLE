...
MODEL_PATH = os.path.join(FIDDLE_DIR, "check_point", "fiddle_tcn_orbitrap_031826.pt")
FDR_MODEL_PATH = os.path.join(
...
# Rescore model (FormulaEncoder + RescoreHead)
rescore_formula_encoder = None
rescore_head = None
if os.path.exists(FDR_MODEL_PATH):
    ckpt = torch.load(FDR_MODEL_PATH, map_location=DEVICE)
    ...
    rescore_head.eval()
    print(f"Loaded rescore model (FormulaEncoder + RescoreHead) from {FDR_MODEL_PATH}")
print("Models ready.\n")
...
def rescore_candidates(z_spec, refined_results, K):
...
def predict(spectrum: dict) -> dict:
    ...
    try:
        ...
        t0 = time.time()
        with torch.no_grad():
            encoded_x, pred_f, pred_mass, _, _ = model(spec_t, env_t)
        pred_f -= na_t
        import torch.nn.functional as F

        z_spec = F.normalize(encoded_x[0], dim=0)
        ...
        refined = rescore_candidates(z_spec, refined, pp["top_k"])
